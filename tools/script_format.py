...
log_grey = config.colours["log_grey"]
...
def fix_script(script_name: str):
    ...
    if meta_index == -1:
        script_data.append({"id": "_meta", "name": format_name(script_name, False)})
    else:
        script_data[meta_index]["name"] = format_name(script_data[meta_index]["name"], False)
